...
public class LL1Checker {
    ...
    private static List<String> tokenize(String input) {
        ...
        while (i < input.length()) {
            ...
            if (Character.isLetter(ch)) {
                ...
                if (Arrays.asList("xaropinho", "xarope", "letra", "uepa", "shi", "ui",
                                "ratinho", "pare", "cavalo", "rapaz", "volta", "danca",
                                "ou", "e").contains(identifier)) {
    ...
    public static void main(String[] args) {
        ...
        // statement → declaration | if_statement | while_loop | function_decl | return_stmt | break_stmt | continue_stmt | block | expression_stmt
        grammar.put("statement", Arrays.asList(
                ...
                Arrays.asList("block"),
                Arrays.asList("IDENTIFIER", "assignment_or_call"),
                Arrays.asList("non_id_expression", ";")));

            grammar.put("assignment_or_call", Arrays.asList(
                Arrays.asList("=", "expression", ";"),
                Arrays.asList("(", "args_opt", ")", ";"),
                Arrays.asList("binary_operator", "expression", ";"),
                Arrays.asList(";")
            ));

            grammar.put("stmt_identifier_tail", Arrays.asList(
                Arrays.asList("=", "expression", ";"),
                Arrays.asList("(", "args_opt", ")", ";"),
                Arrays.asList(";")
            ));

            // non_id_expression handles expressions that don't start with IDENTIFIER
            grammar.put("non_id_expression", Arrays.asList(
        ...
        grammar.put("if_tail", Arrays.asList(
                Arrays.asList("ui", "(", "expression", ")", "shi", "block"), // else if
        ...
        // Missing operators
        grammar.put("unary_operator", Arrays.asList(
                Arrays.asList("danca"),
                Arrays.asList("-")));

        grammar.put("binary_operator", Arrays.asList(
                Arrays.asList("+"),
                Arrays.asList("-"),
                Arrays.asList("*"),
                Arrays.asList("/"),
        ...
        if (checker.isLL1()) {
            ...
            // Testa parsing do arquivo entrada.txt
            try {
                java.nio.file.Path path = java.nio.file.Paths.get("entrada.txt");
                String content = java.nio.file.Files.readString(path);
                System.out.println("\nConteudo do arquivo entrada.txt:");
                ...
                System.out.println("\nIniciando parsing...");
                boolean parseResult = checker.parse(tokens);

                if (parseResult) {
                    System.out.println("SUCESSO: O arquivo entrada.txt foi aceito pela gramatica!");
                } else {
                    System.out.println("ERRO: O arquivo entrada.txt foi rejeitado pela gramatica.");
                }

            } catch (java.io.IOException e) {
                System.out.println("Erro ao ler o arquivo entrada.txt: " + e.getMessage());
            }
